...
} = require("../../models/bookingdrivers.js");
...
const {
  jobAndBookingTable,
} = require("../../lineComponents/jobAndBookingTable.js");
const { afterConfirm } = require("../../lineComponents/afterConfirm.js");
const {
  storeChatMessages,
} = require("../../controllers/chattingController.js");
const he = require("he");
          ...
          switch (params.get("type")) {
            ...
            case "cancel":
              if (params.get("value") === "cancel") {
                ...
                await updateBookingDB(params.get("bookingId"), {
                  bookingStatus: "canceled",
                });
                await updateBookingdriverByBookingId(params.get("bookingId"), {
            ...
            case "selectDriver":
              ...
              if (params.get("value") === "select") {
                await userController.selectDriver(
            ...
            case "confirmInfo":
              ...
              await pushMessage(
                [
                  flexWrapper(
                    afterConfirm(
                      (booking.id + 300000).toString().substring(0, 3) +
                        "-" +
                        (booking.id + 300000).toString().substring(3),
                      roomId
                    )
                  ),
                ],
              ...
              try {
                ...
                const greeting = {
                  ...
                  translated:
                    "Thank you for booking! We are Thai-Taxi operators(Driver Team Leader) and very pleased to serve you.\n\nIf you have any requests or issues, feel free to let us know.",
                };
                const translated = await translations(
                  greeting.translated,
                  "th"
                );
                greeting.message = he.decode(
            ...
            case "currentBooking":
              {
                const bookings = await getCurrentBookingsDB(
                  event.source.userId
                );
                if (bookings.length) {
                  await pushMessage(
                  ...
                  await pushMessage(
                    [textTemplate("Sorry, you have no booking yet.")],
                    "user",
                    event.source.userId
                  );
                }
            ...
            case "currentBookingInfo":
              await userController.getCurrentBooking(
                event.source.userId,
                params.get("bookingId")
              );
              break;
            ...
            case "bookingHistory":
              {
                const bookings = await getAllBookingsByUserIdDB(
                  event.source.userId
                );
                if (bookings.length) {
                  await pushMessage(
            ...
            case "bookingHistoryInfo":
              await userController.getBookingHistory(
                event.source.userId,
                params.get("bookingId")
              );
              break;
    ...
    fulfillmentMessages: [
      {
        text: {
          text: [
            ...
            result || events.length === 2 ? textRespond : "no service",
          ],
        },
      },
    ],
...
const priceCheck = async (reqResult, events) => {
  ...
  const areaType = results.plus_code.compound_code
    .split(" ")[1]
    .replace(/[^a-zA-Z0-9 ]/g, "")
    .toLowerCase();
  ...
  const areaResult = areaCalculation(
    [results.geometry.location.lng, results.geometry.location.lat],
    areaType
  );
