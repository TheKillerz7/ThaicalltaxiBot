...
const {
  driverRegisteredCard,
} = require("../lineComponents/driverRegisteredCard");
...
const driverRegisterToBooking = async (req, res) => {
  try {
    ...
    const cardsWrapped = flexWrapper(
      confirmInfo(booking, prices, data.course, data),
      "Price has arrived"
    );
    ...
    await pushMessage(
      [textTemplate("Registration successful. Waiting for reply.")],
      "driver",
      data.driverId
    );
